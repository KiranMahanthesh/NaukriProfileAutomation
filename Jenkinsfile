pipeline {
    ...
    stages {
        ...
        stage('Checkout Repository') {
            steps {
                echo "[INFO] [$(date /T) $(time /T)] Checking out the repository..."
                checkout scm
            }
        }
        ...
        stage('Cache Cypress') {
            steps {
                echo "[INFO] [$(date /T) $(time /T)] Creating Cypress cache folder if not exists..."
        ...
        stage('Install Dependencies') {
            steps {
                echo "[INFO] [$(date /T) $(time /T)] Installing project dependencies..."
        ...
        stage('Install Cypress Binary') {
            steps {
                echo "[INFO] [$(date /T) $(time /T)] Installing Cypress binary..."
        ...
        stage('Verify Cypress Installation') {
            steps {
                echo "[INFO] [$(date /T) $(time /T)] Verifying Cypress installation..."
        ...
        stage('Run Cypress Tests') {
            steps {
                echo "[INFO] [$(date /T) $(time /T)] Running Cypress tests..."
    ...
    post {
        ...
        failure {
            echo "[ERROR] [$(date /T) $(time /T)] Test run failed. Capturing artifacts..."
            archiveArtifacts artifacts: 'cypress/screenshots/**/*, cypress/videos/**/*', allowEmptyArchive: true
        }

        // Always clean the workspace after execution
        always {
            echo "[INFO] [$(date /T) $(time /T)] Cleaning up workspace..."
            cleanWs()
        }

        // On success, cache the Cypress binary for future runs
        success {
            echo "[INFO] [$(date /T) $(time /T)] Caching Cypress binary for future runs..."
